...
public class Player2Movement : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log("Player2 Script Started");
    ...
    void Update()
    {
        ...
        if (Input.GetAxis("Horizontal2") != 0)
        {
            ...
            horizontalMove = playerSpeed * Input.GetAxisRaw("Horizontal2");
            rb.velocity = new Vector2(horizontalMove, rb.velocity.y);
            animator.SetFloat("Speed", Mathf.Abs(rb.velocity.x));

            if (Input.GetAxis("Horizontal2") > 0 && !facingRight) FlipHorizontal();
            else if (Input.GetAxis("Horizontal2") < 0 && facingRight) FlipHorizontal();
